$(document).ready(function () {
		...
		return new Promise(function (resolve, reject) { //Creating new Promise Chain
			...
			$.get('Message/ownerDetails', function (data) {
				...
				if (dob.length != 0 && addr.length != 0 && phone.length != 0 && bio.length != 0) {
					...
					owenerProfileBio.innerHTML = "Profile Belum Lengkap";
	...
	function setUserDetails(data) {
		...
		(data[0]['bio'].length > 1) ? $('#details_of_bio').html(data[0]['bio']) : $('#details_of_bio').html("--Tidak Ada--");
		...
		(data[0]['dob'].length > 1) ?
			...
			$('#details_of_birthday').html(`<span><i class="fas fa-birthday-cake text-dark pr-2"></i></span>--Tidak Ada--`);

		(data[0]['phone'].length > 1) ?
			$('#details_of_mobile').html(`<span><i class="fas fa-mobile-alt text-dark pr-2"></i></span>${data[0]['phone']}`) :
			$('#details_of_mobile').html(`<span><i class="fas fa-mobile-alt text-dark pr-2"></i></span>--Tidak Ada--`);

		(data[0]['address'].length > 1) ?
			$('#details_of_location').html(`<span><i class="fas fa-map-marker-alt text-dark pr-2"></i></span>${data[0]['address']}`) :
			$('#details_of_location').html(`<span><i class="fas fa-map-marker-alt text-dark pr-2"></i></span>--Tidak Ada--`);
	...
	function offlineOnlineIndicator(data, last_seen) {
		if (data == 'Aktif') {
			$('#name_last_seen p').html("Sedang Aktif");
	...
	function getLastSeen(data) {
		...
		if (days > 0) {
			$('#name_last_seen p').html(`Terakhir kali terlihat ${data}`);
		}
		else {
			(hours > 0) ? $('#name_last_seen p').html(`Terakhir kali di lihat ${hours} jam ${min} menit yang lalu`) :
				(min > 0) ? $('#name_last_seen p').html("Terakhr kali di lihat " + min + " menit yang lalu") :
					$('#name_last_seen p').html("Terakhir kali di lihat sekarang");
		}
	}
			...
			if (jsonData.length == 1) {
				for (var i = 0; i < jsonData.length; i++) {
					if (jsonData[i]['blocked_from'] == unique_id) {
						...
						$('#messageText').attr('disabled', '');
						$('#messageText').attr('placeholder', 'Kamu Telah Memblokir User Ini');
				...
				$('#messageText').attr('placeholder', 'Ketik Disini');
				$('#btn_block').html('Block User');
